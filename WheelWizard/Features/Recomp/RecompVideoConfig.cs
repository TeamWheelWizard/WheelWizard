...
public static class RecompVideoConfig
{
    ...
    /// (auto, d3d11, opengl, ...), but only these actually run the game reliably, so nothing else
    ...
    public static IReadOnlyList<string> OfferedGraphicsApis { get; } =
        RecompPlatform.Current switch
        {
            RecompHostPlatform.Windows => ["d3d12", "vulkan"],
            RecompHostPlatform.MacOS => ["metal"],
            _ => ["vulkan"],
        };
