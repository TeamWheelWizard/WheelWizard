...
public class SettingsManager
{
    ...
    public static Setting USER_FOLDER_PATH = new WhWzSetting(typeof(string), "UserFolderPath", "")
        .SetValidation(value =>
        {
            ...
            var dolphinLocation = DOLPHIN_LOCATION.Get() as string ?? string.Empty;
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux) && PathManager.IsLinuxDolphinConfigSplit())
            {
                ...
                foreach (var requiredSubdirectory in requiredSubdirectories)
                {
                ...
                var environmentVariableToAvoid = "DOLPHIN_EMU_USERPATH";
    ...
    private static readonly Setting DOLPHIN_COMPILATION_MODE = new DolphinSetting(typeof(DolphinShaderCompilationMode), ("GFX.ini", "Settings", "ShaderCompilationMode"),
        DolphinShaderCompilationMode.Default);
    private static readonly Setting DOLPHIN_COMPILE_SHADERS_AT_START = new DolphinSetting(typeof(bool), ("GFX.ini", "Settings", "WaitForShadersBeforeStarting"), false);
    private static readonly Setting DOLPHIN_SSAA = new DolphinSetting(typeof(bool), ("GFX.ini", "Settings", "SSAA"), false);
    private static readonly Setting DOLPHIN_MSAA = new DolphinSetting(typeof(string), ("GFX.ini", "Settings", "MSAA"), "0x00000001").SetValidation(
        value => ( value?.ToString() ?? "") is "0x00000001" or "0x00000002" or "0x00000004" or "0x00000008");
    #endregion

    #region Virtual Settings
    private static double s_internalScale = -1.0;
    public static Setting WINDOW_SCALE = new VirtualSetting(typeof(double),
                                                            value => s_internalScale = (double)value!,
                                                            () => s_internalScale == -1.0 ? SAVED_WINDOW_SCALE.Get() : s_internalScale
    ...
    private static RrGameMode s_internalRrGameMode = RrGameMode.RETRO_TRACKS;
    #endregion
