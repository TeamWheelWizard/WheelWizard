...
public class ApplicationStartupTests
{
    ...
    [InlineData("request")]
    [InlineData("updates")]
    public void ShutdownAfterRequestCompletion_DoesNotBeginTheNextStartupStep(string step)
    {
        var fixture = new Fixture();
        var completed = new TaskCompletionSource<WheelWizard.Shared.OperationResult>();
        if (step == "reload")
            fixture.Mods.ReloadAsync().Returns(completed.Task);
        else if (step == "request")
            fixture.Requests.HandleAsync(Arg.Any<string>()).Returns(completed.Task);
        else
            fixture.Updates.CheckForUpdatesAsync().Returns(completed.Task);
        using var shutdown = new CancellationTokenSource();
        var context = new QueuedContext();
        Task run = Task.CompletedTask;
        context.Run(() => run = fixture.Startup.RunAsync(new StartupOptions("wheelwizard://123", true), shutdown.Token));
        var previous = SynchronizationContext.Current;
        SynchronizationContext.SetSynchronizationContext(null);
        try
        {
            completed.SetResult(Ok());
        }
        finally
        {
            SynchronizationContext.SetSynchronizationContext(previous);
        }
        context.Run(shutdown.Cancel);
        context.Drain();
        Assert.True(run.IsCompletedSuccessfully);
        if (step == "reload")
            fixture.Requests.DidNotReceiveWithAnyArgs().HandleAsync(default!);
        if (step != "updates")
            fixture.Updates.DidNotReceive().CheckForUpdatesAsync();
        fixture.Badges.DidNotReceive().LoadBadgesAsync();
        fixture.Live.DidNotReceive().Start();
        fixture.Launch.DidNotReceiveWithAnyArgs().LaunchAsync(default);
    }
